    ...
    try:
        for index, task in enumerate(tasks):
            ...
            try:
                ...
                # --- STEP 2: DOWNLOAD LOGIC (YouTube vs Telegram) ---
                ...
                # B. YOUTUBE VIDEO
                    ...
                    ydl_opts = {
                        'format': 'best',
                        'outtmpl': f'{batch_folder}/yt_%(id)s.%(ext)s', # Temp name pattern
                        'noplaylist': True,
                        'quiet': True
                    }
                    
                    # Run YT Download in Thread (Blocking na ho)
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                # --- FINAL ENTRY FORMAT ---
                # Monospace Link aur Size neeche
                entry = (
                    f"📂 **{display_name}**\n"
                    f"🔗 `{final_link}`\n"
                    f"📦 Size: {file_size_mb:.2f} MB"
                ...
                # Local files delete karo turant (Space bachane ke liye)
        ...
        # Pura batch folder saaf karo
        if os.path.exists(batch_folder): shutil.rmtree(batch_folder)
        
        # Status Message Delete karo (Taaki sirf List bache)
